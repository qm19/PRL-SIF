...
def label_mechanism_error(data, label, positive_num=200, train_ratio=0.8, error_ratio=0.2, type='LB', esitimator='Linear'):
    ...
    X_label_error_df = pd.DataFrame(X_train).sample(label_error_num, weights=label_error_pro, random_state=seed)
...
def label_error(data, label, data_raw, error_ratio=0.2, train_ratio=0.8, esitimator='Linear'):
    # 对raw sensor data进行标记，获得PN数据集，并划分训练集和测试集，并对训练集的label添加噪声
    true_class = -np.array(label) * 2 + 1  # 将0（正常状态）转换成1，将1（故障状态）转换为-1
    X_train, X_test, Y_train, Y_test = train_test_split(data, true_class, test_size=1 - train_ratio,
                                                        random_state=seed)
    X_train_raw, X_test_raw, Y_train_raw, Y_test_raw = train_test_split(data_raw, true_class, test_size=1 - train_ratio,
    ...
    if error_ratio == 0:
        ...
        label_as_positive = 1 - model.predict_proba(X_train)[:, 1]
        label_error_pro = [label_as_positive[i] if Y_train[i] == -1 else 1-label_as_positive[i] for i in range(len(Y_train))]
        label_error_num = int(len(Y_train) * error_ratio)
        X_label_error_df = pd.DataFrame(X_train).sample(label_error_num, weights=label_error_pro, random_state=seed)
        L_train = np.array([-Y_train[i] if i in X_label_error_df.index else Y_train[i] for i in range(len(Y_train))])
...
def PCA_trans(data, n_component=4):
    data_transform = []
    for i in range(data.shape[2]):
        x = data[:, :, i]
        pca = PCA(n_components=n_component)
        X_pca = pca.fit_transform(x)
        data_transform.append(X_pca.T)
