...
class DEC(object):
    ...
    def compile(self, loss=[CLoss], optimizer='adam'):
    ...
    def fit(self, x, epochs=200, batch_size=16, update_interval=40):
        ...
        for ite in range(math.ceil(epochs/update_interval)):
            ...
            self.model.fit(x, p, steps_per_epoch=batch_size, epochs=update_interval, shuffle=True,
...
if __name__ == "__main__":
    ...
    saveFile_name = '../Results/DEC_' + fileName + '_result.xls'
    ...
    for num in list:
        save_ae_dic = './saveWeights/DEC_AE_' + str(num) + '.h5'
        save_classifier_dic = './saveWeights/DEC_' + fileName + '_' + str(num) + '.h5'
        if Train:
            ae_weights = None
        ...
        # Define DEC model
        dec = DEC(input_dims=x.shape[1], n_clusters=2)
        # plot_model(dec.model, to_file='benchmark5.png', show_shapes=True)
        dec.model.summary()
        t0 = time()
        ...
        dec.compile(loss=[dec.CLoss], optimizer=optimizer)
        if classifier_weights is None:
            dec.fit(x, batch_size=batch_size, epochs=train_epochs, update_interval=update_interval)
            dec.model.save_weights(save_classifier_dic)
        else:
            dec.load_weights(classifier_weights)

        # Show the final results
        y_pred_train = dec.model.predict(x)[:,1]
        y_pred_test = dec.model.predict(x_test)[:,1]
