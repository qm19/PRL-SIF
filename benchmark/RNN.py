...
class RNN(object):
    def __init__(self,
        ...
        self.model.add(LSTM(20, input_shape=self.input_dims, activation='relu', name='input_and_lstm1', return_sequences=True))
        self.model.add(LSTM(10, input_shape=self.input_dims, activation='relu', name='lstm2'))
        self.model.add(Dense(2, name='label_prediction', activation=act_fun,
                             kernel_regularizer=regularizers.l2(self.l2_para),
                             bias_regularizer=regularizers.l2(self.l2_para)))
        self.y_pred = []
...
def parse():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage='RNN.py --method <string> --label_error_ratio <integer> --dataset_name <string>' +
              '--train <bool> --label_method <string> --act_func <string> --l2_weight <float>' +
              '--batchsize <integer> --epoch <integer> --runs <integer> --save <bool>'
    )
    parser.add_argument('--method', type=str, default='RNN', choices=['RNN'],
    ...
    parser.add_argument('--act_func', type=str, default='softmax', choices=['softmax', 'sigmoid', 'relu'],
...
if __name__ == '__main__':
    ...
    for i in range(args.runs):
        save_PU_dic = './saveWeights/' + args.method + '_' + args.dataset_name + '_LE_' + str(args.label_error_ratio) \
                      + '_' + str(i) + '.h5'
        if args.train:
            pu_weights = None
        else:
        ...
        start_time = clock()
        rnn = RNN(input_dims=[X_train.shape[1], X_train.shape[2]], act_fun=args.act_func, l2_parameter=args.l2_weight)
        if i == 0:
            rnn.model.summary()
        ...
        if pu_weights is None:
            rnn.fit(X_train, L_train, batch_size=args.batchsize, train_epochs=args.epoch)
            rnn.model.save_weights(save_PU_dic)
        else:
            rnn.load_weights(pu_weights)

        # Get the prediction results
        f_train = rnn.model.predict(X_train)[:, 1]
        f_test = rnn.model.predict(X_test)[:, 1]
    ...
    if args.save:
        classifier_result_save(df_scores,
                               '../Results/' + args.method + '_' + args.dataset_name + '_LE_' + str(
                                   args.label_error_ratio) + '.xls')
