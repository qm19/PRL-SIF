...
def max_bepp(k):
    def fun(counts):
        ...
        maximum = max(map(lambda TP : (0 if TP[0]==0 else float(TP[1])/(TP[0]+k)), counts)) # <------ I PUT 5 INSTEAD OF K!
...
def Result_save(score, saveFile_name):
    ...
    score_df.columns = ['label_num', 'TIcE_mean', 'TIcE_std', 'alpha_mean', 'alpha_std']
...
if __name__=='__main__':
    ...
    saveFile_name = '../Results/TIcE_' + fileName + 'result.xls'
    repeat_time = 10
    ...
    for num in label_num:
        ...
        # load dataset
        data, labels = dataset_process(dataPath=dataPath)
        c_estimate, alpha = [], []
        for i in range(repeat_time):
            c_estimate_temp, alpha_temp = run_from_elsewhere(data, labels, k=10, its=10)
            # print("c_estimate={}".format(c_estimate))
            # print("alpha_estimate={}".format(alpha))
            c_estimate.append(c_estimate_temp)
            alpha.append(alpha_temp)
        score[m, :] = [num, np.mean(c_estimate), np.std(c_estimate), np.mean(alpha), np.std(alpha)]
        m += 1
