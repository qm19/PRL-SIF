...
if __name__ == "__main__":
    ...
    label_mechanism = 'SCAR'
    piror_estimate_method = 'KM'
    Train = True
    score = np.empty((len(dataset_name_list), 7))
    m = 0
    ...
    saveFile_name = '../Results/nnPU_' + label_mechanism + '_' + piror_estimate_method + '_24_result.xls'

    for dataset_name in dataset_name_list:
        dataPath = '../Data/' + label_mechanism + '_' + dataset_name + '_24'
        # load dataset
        ...
        X_mixture = x[y == -1]
        X_component = x[y == 1]
        prior, _ = wrapper(X_mixture, X_component)

        save_nnPU_dic = './saveWeights/nnPU_' + label_mechanism + '_' + dataset_name + '_' + piror_estimate_method + \
                        '_24.h5'
        if Train:
            nnpu_weights = None
        else:
